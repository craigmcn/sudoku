...
- **[src/firebase.ts](src/firebase.ts)** — Initializes the Firebase app/auth/db from `VITE_FIREBASE_*` env vars, guarded against re-`initializeApp()`.
- **[src/stats.ts](src/stats.ts)** — `ensureAnonymousAuth()` (idempotent `signInAnonymously`, memoized as a module-level promise), `recordPuzzleStart(puzzleId, difficulty, puzzle, solutionHash)` (creates the `puzzles/{puzzleId}` doc on first sight via a transaction, then increments `timesPlayed`), `recordPuzzleCompletion(puzzleId, elapsedMs)` (increments `completions`/`totalPlayTimeMs`). Failures are caught and logged, never block gameplay.
