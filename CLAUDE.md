...
- **[src/stats.ts](src/stats.ts)** — `ensureAnonymousAuth()` (idempotent `signInAnonymously`, memoized as a module-level promise; rejects immediately if `auth` is undefined; does not cache a rejection, so a later call retries), `recordPuzzleStart(puzzleId, difficulty, puzzle, solutionHash)` (creates the `puzzles/{puzzleId}` doc on first sight via a transaction, then increments `timesPlayed`), `recordPuzzleCompletion(puzzleId, elapsedMs)` (increments `completions`/`totalPlayTimeMs`); both throw via `requireDb()` if `db` is undefined. Failures are caught and logged in `main.ts`, never block gameplay.
...
- #15, #16, #17 done. Next up: #18 (daily puzzle) — the last of the four issues split out of #13.
- No open questions blocking #18.
