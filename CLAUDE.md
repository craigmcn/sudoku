...
- **[src/stats.ts](src/stats.ts)** — `ensureAnonymousAuth()` (idempotent `signInAnonymously`, memoized as a module-level promise; rejects immediately if `auth` is undefined; does not cache a rejection, so a later call retries), `recordPuzzleStart(puzzleId, difficulty, puzzle, solutionHash)` (calls `ensurePuzzleDoc`, then increments `timesPlayed`), `recordPuzzleCompletion(puzzleId, elapsedMs)` (increments `completions`/`totalPlayTimeMs`); both throw via `requireDb()` if `db` is undefined. Failures are caught and logged in `main.ts`, never block gameplay.
...
- #15, #16, #17, #18, #20 done — all sub-issues split out of #13's Firebase puzzle-data plan are implemented, plus #20's follow-up cleanup. #13 (umbrella issue) is closed. #12 (Firebase login + per-user stats, which builds on this groundwork) is the next planned work.
- No open questions blocking #12.
